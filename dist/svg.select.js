    ...
    SelectHandler.prototype.drawCircles = function() {
      ...
      for (var i = 0, len = array.length; i < len; ++i) {
        ...
        this.pointSelection.set.add(
          ...
          this.nested
            .rect(this.options.size, this.options.size)
      ...
      if (this.options.points && !this.rectSelection.set.get(1)) {
        ...
        this.rectSelection.set.add(
          this.nested
            ...
            .on(mname, getMoseDownFunc('rt'))
            .on(ename, getMoseDownFunc('rt'))
        );
        this.rectSelection.set.add(
          this.nested
            .rect(this.options.size, this.options.size)
            .radius(this.options.radius)
            .center(bbox.width, bbox.height)
        ...
        this.rectSelection.set.add(
          this.nested
            .rect(this.options.size, this.options.size)
            .radius(this.options.radius)
            .center(0, bbox.height)
            .attr('class', this.options.classPoints + '_lb')
            .on(mname, getMoseDownFunc('lb'))
            .on(ename, getMoseDownFunc('lb'))
        );
        ...
        this.rectSelection.set.add(
          this.nested
            .rect(this.options.size, this.options.size)
            .radius(this.options.radius)
            .center(bbox.width / 2, 0)
            .attr('class', this.options.classPoints + '_t')
            .on(mname, getMoseDownFunc('t'))
            .on(ename, getMoseDownFunc('t'))
        );
        this.rectSelection.set.add(
          this.nested
            .rect(this.options.size, this.options.size)
            .radius(this.options.radius)
            .center(bbox.width, bbox.height / 2)
            .attr('class', this.options.classPoints + '_r')
            .on(mname, getMoseDownFunc('r'))
            .on(ename, getMoseDownFunc('r'))
        );
        this.rectSelection.set.add(
          this.nested
            .rect(this.options.size, this.options.size)
            .radius(this.options.radius)
            .center(bbox.width / 2, bbox.height)
            .attr('class', this.options.classPoints + '_b')
            .on(mname, getMoseDownFunc('b'))
            .on(ename, getMoseDownFunc('b'))
        );
        this.rectSelection.set.add(
          this.nested
            .rect(this.options.size, this.options.size)
            .radius(this.options.radius)
            .center(0, bbox.height / 2)
            .attr('class', this.options.classPoints + '_l')
            .on(mname, getMoseDownFunc('l'))
            .on(ename, getMoseDownFunc('l'))
        );
